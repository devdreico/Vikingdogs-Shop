...
import { ArrowRight, Check, ChevronDown, Menu, Minus, PackageCheck, PawPrint, Plus, ShoppingBag, Sparkles, Truck, X } from 'lucide-react'
...
function Home() {
  usePageSeo({
    title: 'VikingDogs · Cuidado para tu mejor amigo',
    description: 'Productos prácticos para perros: cuidado, aseo, hidratación y aventuras. Envío gratis en toda Colombia.',
    path: '/',
    jsonLd: [
      {
        '@context': 'https://schema.org',
...
function Catalog() {
  ...
  const comingSoonCount = PRODUCTS.filter((product) => product.comingSoon).length
  const noteTitle = comingSoonCount > 0 ? `${comingSoonCount} novedades en camino.` : 'Envío gratis en todo el catálogo.'
  const noteText = comingSoonCount > 0 ? 'Próximamente disponibles · Envío gratis.' : `${PRODUCTS.length} productos disponibles · Sin costo de envío.`
  const itemListLd = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
  ...
  return <Layout><section className="catalog-hero container"><div><span className="eyebrow"><PawPrint size={14} /> VikingDogs</span><h1>Elige su<br /><em>cuidado.</em></h1></div><p>10 productos · Envío gratis · Consulta disponibilidad</p></section>
...
function ProductPage() {
  ...
  usePageSeo({
    ...
    description: product ? `${product.detail} Envío gratis en todos los pedidos.` : 'Producto VikingDogs.',
...
function Checkout({ product, quantity, cart, total, standalone = false, onClose, onSuccess }) {
  ...
  const submit = async (event) => {
    ...
    const payload = { ...form, _replyto: form.email, phone: `+57 ${form.phone}`, _subject: `Nuevo pedido VikingDogs — ${form.firstName} ${form.lastName}`, products: summary, total: formatCOP(total), shipping: 'Envío gratis', source: 'Tienda online VikingDogs' }
